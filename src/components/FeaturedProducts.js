...
function FeaturedProducts() {
  ...
  const selectedCategory = useSelector(
    (state) => state.categoryR.selectedCategory,
  );
  if (productState.arrProducts.length === 0) {
    ...
    if (selectedCategory === "Categories") {
      ...
      filterProducts = productState.arrProducts.filter(
        (e) => e.category.toUpperCase() === selectedCategory.toUpperCase(),
      );
    }
        ...
        <div className='container-fluid pt-5 pb-3'>
          ...
          <div className='row px-xl-5'>
            {filterProducts.map((e, i) => (
              <FeaturedSingleProduct key={i} product={e} />
            ))}
          </div>
        </div>
        {/* Products End */}
