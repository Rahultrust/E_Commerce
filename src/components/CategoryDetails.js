...
function getFilteredProductsOfCategory(categoryName, arrProduct) {
  var arrFilteredProduct = arrProduct.filter(
...
function CategoryDetails(props) {
  ...
  if (productState.arrProducts.length === 0) {
    ...
    var arrFilter = getFilteredProductsOfCategory(
      props.category.name,
      productState.arrProducts,
    );
    ...
    return (
      <div
        onClick={() => {
          dispatch(changeCategory(props.category.name));
        }}
